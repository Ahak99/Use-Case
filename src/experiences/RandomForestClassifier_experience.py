...
def RandomForestClassifier_experience(X_train, X_test, y_train, y_test):
    EXPERIMENT_NAME = "RandomForestClassifier - Experiment"
    EXPERIMENT_ID = mlflow.create_experiment(EXPERIMENT_NAME)
    # current_experiment=dict(mlflow.get_experiment_by_name(EXPERIMENT_NAME))
    # EXPERIMENT_ID=current_experiment['experiment_id']

    # Elements of experience
    learning_rate = [random.uniform(0.1, 0.3) for _ in range(20)]
    ...
    for e in n_estimators:
            for d in max_depth:
                parm_list.append([e, d])

    PARAMS = {}
    MODELS = {}
    for i in range(len(parm_list)):
        PARAMS[i+1] = {"n_estimators" : parm_list[i][0], "max_depth" : parm_list[i][1]}
        MODELS[i+1] = {"model": RandomForestClassifier(n_estimators = parm_list[i][0], max_depth = parm_list[i][1])}
...
if __name__ == "__main__":
    ...
    RandomForestClassifier_experience(X_train, X_test, y_train, y_test)